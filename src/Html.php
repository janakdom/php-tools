...
class Html
{
     ...
     * @param bool   $blankTarget
     * @return string
     */
    static function processLinksInText(string $input, bool $stripTags = false, bool $blankTarget = true) :string
    {
        if($stripTags) {
        ...
        $input = preg_replace("~([a-zA-Z]{2,10}):\/\/([^<>,\s\)]+)~i",
            "<a href=\"\\0\"" . ($blankTarget ? " target=\"_blank\"" : "") . ">\\0</a>", $input);
     ...
     * @param string $ending       Ukončení, které se přilepí na konec zkráceného řetězce. TRUE = použít &amp;hellip;,
     *                             tj. trojtečku.
     ...
     */
    static function shortenHtml(string $text, int $length = 100, $ending = true, bool $exact = false, bool $considerHtml = true) :string
    {
        if($ending === true) {
            $ending = "&hellip;";
        ...
        if($considerHtml) {
            ...
            foreach($lines as $line_matchings) {
                ...
                if(!empty($line_matchings[1])) {
                    ...
                    if(preg_match('/^<(\s*.+?\/\s*|\s*(img|br|input|hr|area|base|basefont|col|frame|isindex|link|meta|param)(\s.+?)?)>$/is',
                        ...
                        if(preg_match('/^<\s*\/([^\s]+?)\s*>$/s', $line_matchings[1], $tag_matchings)) {
                            ...
                            if(preg_match('/^<\s*([^\s>!]+).*?>$/s', $line_matchings[1],
                                $tag_matchings)) {//<? //Toto je kvůli mému blbému editoru, který si myslí, že je ukončeno PHP
                ...
                $content_length = Strings::length(preg_replace('/&[0-9a-z]{2,8};|&#[0-9]{1,7};|[0-9a-f]{1,6};/i', ' ',
                    $line_matchings[2]));
     ...
     * @return string
     */
    static function escape(string $input, bool $doubleEncode = false) :string
    {
     ...
     */
    public static function diff(string $old, string $new,
        string $startIns = "<ins>", string $endIns = "</ins>", string $startDel = "<del>", string $endDel = "</del>") :string
    {
        $ret = '';
        $diff = Arrays::diff(preg_split("/[\s]+/", $old), preg_split("/[\s]+/", $new));
        foreach($diff as $k) {
            if(is_array($k)) {
                $ret .= (!empty($k['d']) ? $startDel . implode(' ', $k['d']) . $endDel . " " : '') . (!empty($k['i']) ?
                        $startIns . implode(' ', $k['i']) . $endIns . " " : '');
            } else {
                $ret .= $k . ' ';
            }
        }
