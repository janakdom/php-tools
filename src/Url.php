...
class Url
{
     ...
     *
     * @param array  $params       Asociativní pole [název-parametru] => hodnota-parametru. Null pro odstranění daného
     *                             parametru, byl-li tam. Hodnota může být array, pak se automaticky převede na několik
     *                             samostatných parametrů ve tvaru název-parametru[]
     ...
     * @param bool   $clearArrays  Jak se má chovat, když v $originalUrl je nějaký parametr - pole (s [] na konci) a v
     *                             $params také. False = sloučit obě pole. True = smazat to z $originalUrl a nechat jen
     *                             hodnoty z $params.
     * @param array  $keepEntities Array entit, které se nemají enkódovat (např. %id%)
     * @return string
     */
    static function builder(array $params, string $originalUrl, bool $clear = false, bool $clearArrays = true, array $keepEntities = []) :string
    {
        $origParams = [];
        if(!$clear) {
     ...
     * @return string
     */
    static function absolutize(string $url, string $baseUrl) :string
    {
     ...
     * @param string $predadresa
     * @return string
     * @ignore
     */
    static protected function absolutizeStep(string $vstup, string $atribut, string $predadresa) :string
    {
        $poz = 0;
        $kontrola = 0;
        while($poz !== false and $poz < Strings::strlen($vstup) and $kontrola < 100) {
            ...
            if(Strings::substr($vstup, $poz_hodnoty, 1) == "\"" or Strings::substr($vstup, $poz_hodnoty, 1) == "'") {
                $poz_hodnoty++;
            }
            if(Strings::substr($vstup, $poz_hodnoty, 7) == "http://" or Strings::substr($vstup, $poz_hodnoty,
                    7) == "mailto:" or Strings::substr($vstup, $poz_hodnoty, 6) == "ftp://" or Strings::substr($vstup,
                    $poz_hodnoty, 8) == "https://") {
                continue;
            }
            if(Strings::substr($vstup, $poz_hodnoty, 1) == "%") {
                continue;
            }
